...
class Audio extends Component {
  ...
  switchSong(musicIndex) {
    const { tracks, currentSource } = this.state;
  ...
  framerDrawTick(x1, y1, x2, y2) {
    ...
    gradient.addColorStop(0.5, "#61abff");
    gradient.addColorStop(1, "#61abff");
  ...
  framerGetTickPoints() {
    ...
    const coords = [];
    const step = framerPI / framerCountTicks;
  ...
  trackerIsOusideOfBigCircle(event) {
    const { canvasCx, canvasCy, canvasCoord, sceneRadius } = this.state;
    return (
